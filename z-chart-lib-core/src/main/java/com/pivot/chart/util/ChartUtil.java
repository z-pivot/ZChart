...
public abstract class ChartUtil {
    ...
    /**
     * 注意：如果调用此函数，会将chart从原来的布局中移除添加到一个RelativeLayout中，然后将RelativeLayout放置到原来的布局中，
     * 也就是说，在RecyclerView和其父布局中间添加了一层RelativeLayout，用来盛放RecyclerView和chart
     */
    public static ViewGroup processTitleAndDescView(Chart charView, String title, String leftDesc, String rightDesc, int titleTextSize, int titleTextColor) {
        if (TextUtils.isEmpty(title) && TextUtils.isEmpty(leftDesc) && TextUtils.isEmpty(rightDesc)) {
            return charView;
        }

        ViewGroup group = (ViewGroup) charView.getParent();
        LinearLayout container = new LinearLayout(charView.getContext());
        container.setTag("temp");
        container.setOrientation(LinearLayout.VERTICAL);
        if (group == null) {
            group = new RelativeLayout(charView.getContext());
            ...
            int index = group.indexOfChild(charView);
            group.removeView(charView);
            if (!"temp".equals(group.getTag())) {
                group.addView(container, index, charView.getLayoutParams());
            } else {
                container = (LinearLayout) group;
                container.removeAllViews();
            }
        }

        if (!TextUtils.isEmpty(title)) {
            TextView tvTile = new TextView(charView.getContext());
            //            tvTile.setTextAppearance(charView.getContext(), R.style.TextStyle_BlackMid_Big);
            tvTile.setTypeface(null, Typeface.NORMAL);
            tvTile.setGravity(Gravity.CENTER);
            tvTile.setText(title);
            int padding = (int) charView.getResources().getDimension(R.dimen.dimens_big);
            tvTile.setPadding(padding, padding, padding, padding);
            LinearLayout.LayoutParams titleParams = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
            titleParams.gravity = Gravity.CENTER_HORIZONTAL;
            if (titleTextSize > 0) {
                tvTile.setTextSize(titleTextSize);
            }
            if (titleTextColor != 0) {
                tvTile.setTextColor(titleTextColor);
            }
            container.addView(tvTile, titleParams);
        }

        if (!TextUtils.isEmpty(leftDesc) || !TextUtils.isEmpty(rightDesc)) {
            RelativeLayout relativeLayout = new RelativeLayout(charView.getContext());
            if (!TextUtils.isEmpty(leftDesc)) {
                RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
                params.addRule(RelativeLayout.ALIGN_PARENT_LEFT);
                params.leftMargin = DisplayUtil.dip2px(charView.getContext(), 10);
                TextView tvDesc = new TextView(charView.getContext());
                tvDesc.setTextSize(12);
                tvDesc.setTextColor(charView.getResources().getColor(R.color.black_light));
                tvDesc.setText(leftDesc);
                relativeLayout.addView(tvDesc, params);
            }
            ...
            if (!TextUtils.isEmpty(rightDesc)) {
                ...
                TextView tvDesc = new TextView(charView.getContext());
                tvDesc.setText(rightDesc);
                tvDesc.setTextColor(charView.getResources().getColor(R.color.black_light));
                tvDesc.setTextSize(12);
                relativeLayout.addView(tvDesc, params);
            }
            container.addView(relativeLayout, new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
        }
